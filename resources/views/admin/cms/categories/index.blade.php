          ...
          <div class="col-md-3">
            <div class="create-btn pull-right">
              <a href="{{ route('cms.categories.create') }}"
                 class="btn custom-create-btn">{{ __('cms.category.form.add-button') }}</a>
            </div>
          </div>
        @endif
      </div>
...
@push('scripts')
  <script>
    $(function() {
      $('#table').DataTable({
        ...
        ajax: '{{ route('cms.categories.index') }}',
        columns: [{
            data: 'DT_RowIndex',
            name: 'DT_RowIndex'
          },
  ...
  <script type="text/javascript">
    function changeCmsCategoryStatus(_this, id) {
      ...
      $.ajax({
        url: `{{ route('cms.categories.update_status', '') }}/${id}`,
        type: 'post',
        data: {
          _token: _token,
          _method: 'patch',
